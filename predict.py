...
if __name__ == '__main__':
    ...
    assert os.path.exists(
        options.locator_path), "Provided locator load path doesnt exist"
    assert options.segmentor_path is not None, "Segmentation model load path must be provided during testing mode"
    assert os.path.exists(
        options.segmentor_path), "Provided segmentor load path doesnt exist"
    ...
    use_cuda = torch.cuda.is_available() and options.gpu
    ...
    output_dir = os.makedirs(options.output_dir, exist_ok=True)

    print(f"""Provided locator checkpoint load path: '{options.locator_path}'
                                    and segmentor load path: '{options.segmentor_path}' """)

    locator_checkpoint = torch.load(options.locator_path, map_location='cpu')
    segmentor_checkpoint = torch.load(
        options.segmentor_path, map_location='cpu')
    ...
    print(
        f"Checkpoint locator model weights loaded from resume path: '~/{options.locator_path}'")
    print(
        f"Checkpoint segmentation model weights loaded from resume path: '~/{options.segmentor_path}'")
    print(
    ...
    with torch.no_grad():
        ...
        for index in range(len(test_set)):
            ...
            prediction_file_path = os.path.join(
                options.output_dir, 'prediction_' + raw_image_filename)
            sitk.WriteImage(prediction_image, prediction_file_path)
            print(
                f"Saved prediction for file: {raw_image_filename} to path: {prediction_file_path} ")
